...
public class DbExecutor implements ServiceInvoker{
    ...
    private DbServiceConfigService dbService;
    ...
    public DbExecutor(DbServiceConfigService dbService, DataSourceConfigService dataSourceService, ServiceLogService logService, PlatformTransactionManager txManager) {
    ...
    @Override
    public Response<Object> execute(Long serviceId, String jsonObject) {
        try {
            DbServiceConfig serviceConfig = dbService.getConfigById(serviceId);
            if (serviceConfig == null) {
                return Response.error("ServiceConfig not found: " + serviceId);
            }

            Map<String, Object> inputParams = JsonUtil.toObjectMap(jsonObject);

            return executeSQL(serviceConfig, inputParams);

        } catch (Exception e) {
            return Response.error("Failed to execute SQL: " + e.getMessage());
        }
    }

    private Response<Object> executeSQL(DbServiceConfig serviceConfig, Map<String, Object> inputParams) {
        ...
        try {
            ...
            if (Boolean.TRUE.equals(serviceConfig.getServiceConfig().getLogEnabled())) {
                ServiceLog log = new ServiceLog();
                log.setServiceCode(serviceConfig.getServiceConfig().getServiceCode());
        ...
        if (statusCode == 200) {
            return Response.success(resultData).withLogId(logId);
        } else {
            String msg = errorMsg + (logId != null ? " [Log_ID: " + logId + "]" : "");
            return Response.error(msg).withLogId(logId);
        }
    }
