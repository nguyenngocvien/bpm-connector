...
public class DbServiceRepository {
    ...
    public void save(DbServiceConfig config, Long serviceId) {
        ...
        jdbcTemplate.update(connection -> {
            ...
            ps.setLong(2, config.getDbSourceId());
            ps.setString(3, config.getSqlStatement());
            ps.setObject(4, config.getSqlType().name(), java.sql.Types.VARCHAR); // <-- explicit type
            ps.setString(5, config.getInputParams());
            ps.setString(6, config.getOutputMapping());
            ps.setInt(7, config.getTimeoutMs());
            ps.setInt(8, config.getRetryCount());
            ps.setLong(9, config.getRetryBackoffMs());
            ps.setBoolean(10, config.getTransactional());
            ps.setInt(11, config.getFetchSize());
            ps.setString(12, config.getResultType());
            ps.setBoolean(13, config.getEnabled());
            return ps;
        });
    }
